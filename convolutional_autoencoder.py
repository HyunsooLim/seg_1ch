...
def draw_results(test_inputs, test_targets, test_segmentation, test_accuracy, network, batch_num):
    ...
    for example_i in range(n_examples_to_plot):
        ...
        test_image_thresholded = np.array(
            [0 if x < 0.25 else 255 for x in test_segmentation[example_i].flatten()])
...
def train():
    ...
    dataset = Dataset(folder='data'.format(network.IMAGE_HEIGHT, network.IMAGE_WIDTH), include_hair=True,
